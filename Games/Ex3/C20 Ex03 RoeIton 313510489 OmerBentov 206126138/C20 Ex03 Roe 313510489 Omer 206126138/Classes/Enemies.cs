...
namespace C20_Ex03_Roe_313510489_Omer_206126138.Classes
{
    public class Enemies
    {
        ...
        {
            int distance = isOneOfEnemiesTochesBorder((float)m_GameScreen.GraphicsDevice.Viewport.Width);
        ...
        private bool EnemyReachedBottom()
        {
            ...
            for (int i = 0; i < m_Enemies.GetLength(0); i++)
            {
                for (int j = 0; j < m_Enemies.GetLength(1); j++)
                {
                    ...
                    if (enemy.IsAlive && (enemy.Position.Y + enemy.Texture.Height >= m_GameScreen.GraphicsDevice.Viewport.Height))
                    {
                        answer = true;
                    }
                }
